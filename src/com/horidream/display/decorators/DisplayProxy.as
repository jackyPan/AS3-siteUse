...
package com.horidream.display.decorators {
	...
	/**
	 * copy from TweenProxy 
	 * @author horidream
	 * 
	 */	  
	dynamic public class DisplayProxy extends Proxy {
		/** @private **/
		...
		public function DisplayProxy(target:DisplayObject, ignoreSiblingUpdates:Boolean=false) {
			_target = target;
		...
		public static function create(target:DisplayObject, allowRecycle:Boolean=true):DisplayProxy {
			if (_dict[target] != null && allowRecycle) {
				return _dict[target][0];
			} else {
				return new DisplayProxy(target);
			}
		}
